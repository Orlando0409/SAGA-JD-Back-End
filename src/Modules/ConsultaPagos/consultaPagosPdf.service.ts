...
export class ConsultaPagosPdfService implements OnApplicationShutdown {
  ...
  private logoDataUri: string | null = null;
    ...
    async generarFacturasDesdeConsultas(
      ...
      const html = this.buildFacturaHtml(inputs);
    ...
    private buildFacturaHtml(inputs: FacturaInput[]): string {
        ...
        const sections = inputs.map((input, index) => {
            ...
            const consumosRows = this.buildConsumoRows(input.historialLecturas);
  ...
  <section class="factura ${pageBreakClass}">
    <div class="sheet">
      ...
      <div class="info-grid two-col">
        ...
        <div class="cell"><span>Cliente:</span><strong>${this.escapeHtml(input.nombreCliente)}</strong></div>
        <div class="cell"><span>Identificacion:</span><strong>${this.escapeHtml(input.identificacion)}</strong></div>
        <div class="cell"><span>Tarifa:</span><strong>${this.escapeHtml(input.tipoTarifa)}</strong></div>
        ...
        <div class="panel">
          ...
          <table class="small-table">
            <tbody>
              <tr>
                <td>TARIFA ACTUAL</td>
                <td class="right">${this.escapeHtml(input.tipoTarifa)}</td>
              </tr>
              <tr>
                <td>CARGO FIJO</td>
                <td class="right">CRC ${input.cargoFijo.toLocaleString('es-CR')}</td>
...
<body>
  ...
  private getMes(m: number): string {
